package dev.passarelli.xprison.multipliers.repo.impl;
...
public class MultipliersRepositoryImpl implements MultipliersRepository {
	...
	public void createTables() {
		this.database.executeSql("CREATE TABLE IF NOT EXISTS " + TABLE_NAME + "(UUID varchar(36) NOT NULL UNIQUE, sell_multiplier NUMERIC, sell_multiplier_timeleft BIGINT, primary key (UUID))");
		this.database.executeSql("CREATE TABLE IF NOT EXISTS " + TABLE_NAME_TOKEN + "(UUID varchar(36) NOT NULL UNIQUE, token_multiplier NUMERIC, token_multiplier_timeleft BIGINT, primary key (UUID))");
	}
